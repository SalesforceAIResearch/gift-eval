...
from tsf_models import get_chronos_forecast_fn, get_timesfm_forecast_fn, get_tirex_forecast_fn, get_moirai_forecast_fn
AVAILABLE_MODELS  = {
...
FORECAST_FEATURES = ['median', 'quantile_0', 'quantile_1', 'quantile_2', 'quantile_3', 'quantile_4', 'quantile_5', 'quantile_6', 'quantile_7', 'quantile_8']
...
class TimeSeriesProcessor:
    ...
    def _downsample_time_series(self, history_values, history_timestamps, candidate_preds, model_names, future_timestamps, future_values=None, downsample_step = 1):
        downspl_future_values = future_values[0::downsample_step] if future_values is not None else None
        downspl_future_timestamps = future_timestamps[0::downsample_step]
        downspl_candidate_preds = {}
        for m_name in model_names:
            downspl_candidate_preds[m_name] = candidate_preds[m_name][0::downsample_step]
            
        downspl_history_values = history_values[-downsample_step::-downsample_step][::-1]
        downspl_history_timestamps = history_timestamps[-downsample_step::-downsample_step][::-1]
        return (downspl_history_values, downspl_history_timestamps, downspl_candidate_preds, downspl_future_timestamps, downspl_future_values)
        

    def _truncate_time_series(self, history_values, history_timestamps, candidate_preds, model_names, future_timestamps, max_ctx_length, max_future_length):
        ...
        return truncated_history_values, truncated_history_timestamps, truncated_candidate_preds, truncated_future_timestamps, ctx_length, pred_length
    ...
    def __call__(self, time_series_data):
        ...
        raw_history_timestamps = pd.date_range(start=raw_history_start, end=raw_history_end, freq=raw_history_frequency)
        # raw_future_values = time_series_data['future_seq']
        raw_future_start, raw_future_end = time_series_data['future_window']
        raw_pred_timestamps = pd.date_range(start=raw_future_start, end=raw_future_end, freq=raw_history_frequency)
        ...
        model_names = time_series_data['model_names']
        ...
        for m_name in model_names:
            ...
            _cv_metrics[m_name] = self._compute_mae(_pred, raw_history_values[-raw_pred_length:]) # crossval on the last part of the history values
            norm_candidate_cvs[m_name] = self._normalize_values(_pred, mean, std)
        cv_ranking = [m_name for m_name in sorted(_cv_metrics.keys(), key=lambda x: _cv_metrics[x])]
        ...
        global_history_values, global_history_timestamps, global_candidate_preds, global_future_timestamps, _ = self._downsample_time_series(
            norm_history_values, raw_history_timestamps,
            norm_candidate_preds, model_names, raw_pred_timestamps, None,
            downspl_step,
        )
        max_ctx_length = int(self.max_history_future_ratio * self.max_future_length)
        global_history_values, global_history_timestamps, global_candidate_preds, global_future_timestamps, _, _ = self._truncate_time_series(
            global_history_values, global_history_timestamps, 
            global_candidate_preds, model_names, global_future_timestamps,
            max_ctx_length, self.max_future_length,
        )

        global_candidate_cvs = {}
        for m_name in model_names:
            global_candidate_cvs[m_name] = norm_candidate_cvs[m_name][-downspl_step::-downspl_step][::-1]
        ...
        for m_name in model_names:
            global_candidate_pred_strings[m_name] = ','.join([f'{v:.3f}' for v in global_candidate_preds[m_name]])
        global_candidate_cvs_strings = {}
        for m_name in model_names:
            global_candidate_cvs_strings[m_name] = ','.join([f'{v:.3f}' for v in global_candidate_cvs[m_name]])
        ...
        # for m_name in model_names:
        #     _pred = time_series_data['candidate_preds'][m_name]['median']
        #     _metrics[m_name] = self._compute_mae(_pred, raw_future_values)
        # best_model = self._get_best_model(_metrics)
        # answer = "\\boxed{" + best_model+ "}"
    ...
    def check_preds_diversity(self, all_preds, diversity_threshold=0.0, max_forecast_length=40, history_values=None):
        mean, std = self._get_norm_factor(history_values)
        all_preds = {m_name: self._normalize_values(all_preds[m_name]['median'], mean, std) for m_name in all_preds}
...
def llm_response(messages, tokenizer, model):
        prompt = tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
        llm_inputs = tokenizer([prompt], return_tensors="pt").to(model.device)
        generated_ids = model.generate(**llm_inputs)
...
def _get_mixture_pred(candidate_preds, model_names):
    ...
    for key in feature_keys:
        if key not in FORECAST_FEATURES:
            ...
            if key != 'median':
                k_quantiles = np.array([candidate_preds[m_name][key] for m_name in model_names])
...
def get_best_pred(best_model, candidate_preds, model_names):
    mixture_pred = _get_mixture_pred(candidate_preds, model_names)
    if best_model != 'mixture':
        ...
        for key in FORECAST_FEATURES:
            mixture_pred[key] = (np.array(mixture_pred[key]) + np.array(offset)).tolist()
    return mixture_pred
...
if __name__ == "__main__":
    ...
    # load llm and candidate forecasters
    print("==> Loading candidate forecasters...")
    time_series_forecasters = TimeSeriesForecasters(model_names=CANDIDATE_MODELS)
    print("==> Loading LLM...")
    from transformers import AutoTokenizer, AutoModelForCausalLM
    llm_tokenizer = AutoTokenizer.from_pretrained("Salesforce/moirai-agent")
    llm_model = AutoModelForCausalLM.from_pretrained("Salesforce/moirai-agent", dtype="auto", device_map="auto")

    # get time series information
    from datasets import load_dataset
    from tqdm import tqdm
    test_samples = load_dataset('parquet', data_files={'x': 'ett2_H_long.parquet'}, split='x')
    ...
    for sample_idx in tqdm(range(num_samples)):
        ...
        time_series_data = {
            'history_seq': np.array(example_data['history_value'], dtype='float').tolist(),
            'history_window': [example_data['history_start'], example_data['history_end']],
            'frequency': example_data['frequency'],
            # 'future_seq': np.array(example_data['future_value'], dtype='float').tolist(),
        ...
        # get candidate predictions and cross-validation predictions
        candidate_preds = time_series_forecasters.get_forecasts(
            time_series_data['history_seq'], 
            time_series_data['pred_length']
        )
        candidate_crossval = time_series_forecasters.get_forecasts(
        ...
        time_series_processor = TimeSeriesProcessor(max_future_length=40, max_history_future_ratio=10)
        chat_prompt = time_series_processor(time_series_data)
        ...
        response = llm_response(messages, llm_tokenizer, llm_model)
        
        # get final prediction
        is_diverse = time_series_processor.check_preds_diversity(time_series_data['candidate_preds'], history_values=time_series_data['history_seq'])
        response = time_series_processor.parse_answer(response, time_series_data['model_names']) if is_diverse else 'mixture'

        final_pred = get_best_pred(response, time_series_data['candidate_preds'], time_series_data['model_names'])
        time_series_data['final_pred'] = final_pred
        
        results.append({'sample_idx': sample_idx, 'response': response, 'is_diverse': is_diverse})
        print(results[-1])
